...
try:
    ...
    while 1:
        ...
        if (c == 10 or c == 13) and len(cur_line) > 0:
            ...
            stdscr.addstr(pos_type, pos_con, " cur_line sent      ".encode(
                code), curses.A_STANDOUT)
            ...
            stdscr.addstr(pos_type, pos_con, " no message to send ".encode(
                code), curses.A_STANDOUT)

        elif c == curses.KEY_DC or c == curses.KEY_BACKSPACE or c == 127:
            cur_line = cur_line[0:-1]
            stdscr.addstr(
                pos_type, (pos["x"] + pos_msg + len(cur_line)), " ".encode(code))
            stdscr.addstr(pos_type, pos_con,
                          "                     ".encode(code))

        elif len(cur_line) >= (pos_max):
            stdscr.addstr(pos_type, pos_con, " your at my limit   ".encode(
                code), curses.A_STANDOUT)
